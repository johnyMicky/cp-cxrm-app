...
export default function Leads() {
  ...
  const handleStatusChange = async (leadId: string, newStatus: string) => {
    try {
      const lead = leads.find((item: any) => String(item.id) === String(leadId));
      ...
      // Reuse the existing status-change write as the Performance event source.
      // This stays lightweight: one Lead update + one existing history record,
      // with no extra listener, polling loop or separate per-call Performance write.
      await firestoreService.updateLeadStatusWithPerformance({
        leadId,
        userId: String(currentUser.id || ''),
        userName: localStorage.getItem('userName') || '',
        status: newStatus,
        countsAsAnswered: statusConfig?.countsAsAnswered === true,
        trackPerformance: currentUser.role === 'Agent',
        source: lead?.source || '',
  ...
  // Auto Dialer keeps using the existing row Status selector while awaiting_status.
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-[#0A0F1C] p-4 rounded-xl border border-white/5 shadow-sm">
            ...
            <div className="space-y-1.5 relative" data-leads-filter-dropdown="true">
              ...
              <div className="relative">
                ...
                >
                  <ChevronDown className={`w-3 h-3 transition-transform ${activeDropdown === 'source' ? 'rotate-180' : ''}`} />
                </button>
              </div>
